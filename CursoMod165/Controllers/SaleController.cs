	...
	public class SaleController : Controller
	{
        ...
        public IActionResult AddProductToList(Product product, int id) //devolve ID do Produto a adicionar
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                productList.SaleID = ViewBag.Sale;  //  este tem de vir da vista
        ...
        [HttpPost]
        public JsonResult PostName(ProductList productList, string Qtd, string Order_ID, string Prod_ID) // , string name2
        {

            if (ModelState.IsValid)
            {
                // atualiza viewBag Qtd
                ViewBag.Quantity = Int32.Parse(Qtd);
                // devolve as novas variaveis da vista

                ViewBag.ProductID = Int32.Parse(Prod_ID); // Prod_ID or id
                Product? productx = _context.Products.Find(ViewBag.ProductID);
                productList.Price = productx.Price;

                // atualiza viewBag com ID da Venda
                ViewBag.SaleID = Int32.Parse(Order_ID);  // Order_ID

                // Crio registo Novo
                _context.ProductLists.Add(productList);
                // tens aqui varios pedido agora grava


                // ProductList? productL = _context.ProductLists.Find(ViewBag.OrderID);  // id - ViewBag.OrderID
                // Adiciona novo produto à Lista
                // Coloca a Zero para poder criar Novo
                productList.ID = 0;  // vamos ver se resulta assim
                productList.Quantity = ViewBag.Quantity;
                // productList.Price = 10;  // ViewBag.Price
                productList.SaleID = ViewBag.SaleID;  // ViewBag.Sale 1
                productList.ProductID = ViewBag.ProductID;  // ver como coloco aqui a ref do produto ... 1
                                                            //_context.ProductLists.Update(productList);  // update ???
